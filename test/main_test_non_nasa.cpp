...
// Test encoding of COM2 A0 control packets.
void test_com2_request_encoding()
{
    std::cout << "test_com2_request_encoding" << std::endl;

    // Known state: Cool, 23°C, Fan 2, power on.
    Com2Request cool;
    cool.dst = "20";
    cool.target_temp = 23.0f;
    cool.fanspeed = FanMode::Mid;
    cool.mode = Mode::Cool;
    cool.power = true;

    auto cool_data = cool.encode();
    ...
    assert(cool_data[5] == 0x18);
    assert(cool_data[6] == 0x97); // Fan 2 (0x80) + 23°C offset (0x17)
    assert(cool_data[7] == 0x01); // Cool
    assert(cool_data[8] == 0xF4); // Power on
    assert(cool_data[13] == 0x34);

    // Verify the generated checksum makes the complete packet valid.
    uint8_t crc = cool_data[1];
    for (int i = 2; i < 12; i++)
        crc ^= cool_data[i];
    assert(crc == cool_data[12]);

    // Exercise the other known mode/fan/power encoding.
    Com2Request heat;
    heat.dst = "20";
    heat.target_temp = 20.0f;
    heat.fanspeed = FanMode::High;
    heat.mode = Mode::Heat;
    heat.power = false;

    auto heat_data = heat.encode();

    assert(heat_data[6] == 0xB4); // Fan 3 (0xA0) + 20°C offset (0x14)
    assert(heat_data[7] == 0x04); // Heat
